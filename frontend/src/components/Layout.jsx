    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-white border-r border-gray-200 transform transition-transform lg:translate-x-0 lg:static lg:inset-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center gap-3 h-16 px-6 border-b border-gray-200">
          ...
          <div>
            <h1 className="font-bold text-gray-900 text-sm">Morocco CRM</h1>
            <p className="text-xs text-gray-500">Gestion des factures</p>
          </div>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <Link to="/profile" className="text-sm hover:opacity-75 transition-opacity">
              <p className="font-medium text-gray-900">{user?.first_name || user?.email}</p>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center px-4 lg:px-8">
          <button className="lg:hidden mr-4 text-gray-500" onClick={() => setSidebarOpen(true)}>
